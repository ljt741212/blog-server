...
-- ============================================
-- 17. AI 模型配置表 (ai_configs)
-- ============================================
CREATE TABLE IF NOT EXISTS `ai_configs` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `name` varchar(50) NOT NULL COMMENT '配置名称',
  `provider` varchar(20) NOT NULL COMMENT '提供商：openai/deepseek/anthropic',
  `model` varchar(50) NOT NULL COMMENT '模型标识',
  `api_key` varchar(500) NOT NULL COMMENT 'API Key（加密存储）',
  `base_url` varchar(255) DEFAULT NULL COMMENT 'API 地址（留空用默认）',
...
CREATE TABLE IF NOT EXISTS `ai_usage_logs` (
  ...
  `action` varchar(30) NOT NULL COMMENT '动作：continue_write/polish/summary/title/chat',
  `created_at` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) COMMENT '创建时间',
  PRIMARY KEY (`id`),
