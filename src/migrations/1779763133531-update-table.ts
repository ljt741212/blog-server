...
export class UpdateTable1779763133531 implements MigrationInterface {
  name = 'UpdateTable1779763133531';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`site_config\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`background_image\` varchar(500) NULL COMMENT '背景图URL', PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`visitors\` CHANGE \`ip\` \`ip\` varchar(50) COLLATE "utf8mb4_0900_ai_ci" NOT NULL COMMENT 'IP地址'`,
    );
    await queryRunner.query(
      `ALTER TABLE \`visitors\` DROP INDEX \`IDX_a7910eb5f7936b71500fec8053\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`visitors\` CHANGE \`fingerprint\` \`fingerprint\` varchar(64) COLLATE "utf8mb4_0900_ai_ci" NULL COMMENT '访客指纹（旧字段，兼容用）'`,
    );
    await queryRunner.query(
      `ALTER TABLE \`visitors\` DROP INDEX \`IDX_37ae698fcd524f2ecdd29f83bf\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`visitors\` CHANGE \`visitor_id\` \`visitor_id\` varchar(64) COLLATE "utf8mb4_0900_ai_ci" NULL COMMENT '访客唯一ID（前端 localStorage 中的 visitorId）'`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`users\` CHANGE \`email\` \`email\` varchar(100) COLLATE "utf8mb4_0900_ai_ci" NOT NULL COMMENT '邮箱'`,
    );
    await queryRunner.query(
      `ALTER TABLE \`users\` DROP INDEX \`IDX_fe0bb3f6520ee0469504521e71\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`users\` CHANGE \`username\` \`username\` varchar(50) COLLATE "utf8mb4_0900_ai_ci" NOT NULL COMMENT '用户名'`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`tags\` CHANGE \`name\` \`name\` varchar(50) COLLATE "utf8mb4_0900_ai_ci" NOT NULL COMMENT '标签名称'`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`categories\` CHANGE \`name\` \`name\` varchar(50) COLLATE "utf8mb4_0900_ai_ci" NOT NULL COMMENT '分类名称'`,
    );
    await queryRunner.query(
